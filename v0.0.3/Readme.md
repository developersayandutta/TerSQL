...
[![Version](https://img.shields.io/badge/version-0.0.3--beta-blue.svg)](https://github.com/developersayandutta/TerSQL)
...
- **Natural Language & Auto-fix** — Write "show me users" or "age between 20 and 40" and let the offline regex rule-engine seamlessly structure valid SQL.
...
- **Session statistics** — per-session query counts, row totals, average elapsed time, error counts, and cache hits via `.stats`.
- **Enriched table metadata** — surface engine, size, owner, and collection stats via `.info`.
- **Dialect-aware NLP** — PostgreSQL backends automatically receive `ILIKE` instead of `LIKE` from the NLP engine.
...
| `.schema <table>` | Describe table columns or collections, plus enriched metadata |
| `.info <table>` | Display enriched table metadata (engine, size, owner, collection stats) |
| `.pks <table>` | Display primary key columns for the given table |
| `.plugins` | List all registered database plugins with their metadata |
...
| Command | Description |
|---|---|
| `.stats` | Display the current session statistics summary (queries, rows, avg elapsed, errors, cache hits) |
...
The NLP engine supports an expanding set of natural-language patterns. Filler words (`please`, `can you`, `show me`, `fetch`, `retrieve`) are automatically stripped before intent matching.

```
TerSQL [mysql:testdb] > show me users where age between 20 and 40
→ SELECT * FROM users WHERE age BETWEEN 20 AND 40;

TerSQL [mysql:testdb] > find products where category in electronics, books
→ SELECT * FROM products WHERE category IN ('electronics', 'books');
```

On PostgreSQL backends, `LIKE` patterns are automatically rewritten to `ILIKE` for case-insensitive matching.

Minor typos in table or column names are resolved via fuzzy matching to the closest known name.
...
Map all database responses into the `QueryResult` dataclass (including the `truncated` and `total_count` fields) so TerSQL renders them reliably.

Use `PluginRegistry.list_meta()` to verify your plugin appears in the registry after registration. Plugin metadata is surfaced to users via `.plugins`.
...
- **Large result sets**: Results are fully bundled into memory strings before output renders. A truncation note is appended to the row label when `QueryResult.truncated` is set.
- **Python < 3.10**: Strictly leverages modern dictionary syntax (`|` operator) restricting pre-2021 python engines natively.
- **MongoDB backup**: `pg_dump`-style backup is not available for MongoDB backends; TerSQL will prompt to continue without backup rather than crash.
...
### v0.0.3 (beta) — NLP Enrichment & Metadata Release

- **NLP Enhancements**: `BETWEEN … AND …` and bare `IN` list patterns added to `ConditionParser`; filler-word stripping and expanded contraction normalisation in `NLPEngine`; `translate_batch()` for bulk NL translation; dialect-aware SQL (PostgreSQL receives `ILIKE`); fuzzy column/table name resolution via edit-distance fallback.
- **SafetyGate expansion**: `DROP SCHEMA`, `DROP VIEW`, `DROP FUNCTION`, and `DROP PROCEDURE` are now blocked in both `SafetyChecker` (NLP layer) and `SafetyGate` (Core layer).
- **New dot commands**: `.info <table>`, `.pks <table>`, `.plugins`, `.stats` — plus aliases `.pk`, `.plug`, `.tableinfo`.
- **Session statistics**: `.stats` and `.status` now surface the full `PluginStats` block (total queries, total rows, avg elapsed, errors, cache hits) along with `HealthStatus.warnings` and backend `extra` fields.
- **Enriched schema view**: `.schema` appends `table_info()` metadata block after the column listing.
- **MongoDB `.dbs` fix**: Routes to `db.list_databases()` instead of issuing invalid `SHOW DATABASES` syntax.
- **Graceful backup fallback**: MongoDB backends raising `NotImplementedError` on backup now prompt to continue rather than crash.
- **`QueryResult` extensions**: New `truncated` and `total_count` fields; truncation note rendered in row label when applicable.
- **Dead code removal**: Removed leftover Groq source branch from `Core.py`; `source == "none"` (unmatched NL) handled cleanly without crash.
...
- Base output configurations and `.bookmark` state mappings.
