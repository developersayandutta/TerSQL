#!/usr/bin/env python3
"""
TerSQL.py — CLI entry point v0.0.2
...
DOT_COMMANDS = [
    ".help", ".status", ".tables", ".schema", ".dbs", ".use",
    ...
    def _sync_completer_schema(self):
        ...
        if getattr(self.core, "_schema_cache", None):
            for tbl, cols in self.core._schema_cache.items():
                words.append(tbl)
                words.extend(c.split(".")[-1] for c in cols) # e.g. users.id -> id
    ...
    def _exec_file(self, path: str):
        ...
        try:
            with open(path, encoding="utf-8") as f:
                sql = f.read()
            # Split on ; (naive — sufficient for most dump files)
